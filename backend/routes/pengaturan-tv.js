...
router.get('/display', async (req, res) => {
  try {
    ...
    const [videos] = await db.execute(
      "SELECT * FROM tv_video WHERE tampil = 'Ya' ORDER BY created_at DESC"
    );
...
router.get('/video', async (req, res) => {
  try {
    const db = await getDatabase();
    const [rows] = await db.execute(
      'SELECT * FROM tv_video ORDER BY created_at DESC'
    );
    res.json(rows);
  } catch (error) {
...
router.post('/video', async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      'INSERT INTO tv_video (link_video, tampil, deskripsi) VALUES (?, ?, ?)',
      [link_video, tampil || 'Ya', deskripsi || null]
    );
...
router.put('/video/:id', async (req, res) => {
  try {
    ...
    await db.execute(
      'UPDATE tv_video SET link_video = ?, tampil = ?, deskripsi = ? WHERE id = ?',
      [
