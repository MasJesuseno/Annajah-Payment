...
import { getGuruById, deleteGuru, uploadFotoGuru, deleteFotoGuru, getKelasByWali, createRiwayatPendidikan, updateRiwayatPendidikan, deleteRiwayatPendidikan } from '../api'
import {
  ArrowLeft, Edit2, Trash2, User, Phone, MapPin,
  Calendar, BookOpen, CreditCard, Hash, Award, AlertCircle,
  School, Loader2, Camera, ZoomIn, Crop, X, GraduationCap,
  Users, Plus, Save, GraduationCap as Diploma, BookOpen as BookIcon
} from 'lucide-react'
...
export default function DetailGuru() {
  ...
  const [riwayatForm, setRiwayatForm] = useState({
    id_guru: '', jenjang: 'S1', nama_sekolah: '', jurusan: '',
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      try {
        await uploadFotoGuru(id, croppedFile)
  ...
  const handleDelete = async () => {
    if (!confirm('Yakin ingin menghapus karyawan ini? Semua data terkait dan akun loginnya akan dihapus.')) return
    try {
      await deleteGuru(id)
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span className="text-sm font-medium">Kembali ke Data Karyawan</span>
        </button>
      ...
      <div className="card overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-5">
            ...
            <div className="text-white text-center sm:text-left pb-1 flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1 text-white/80 text-sm">
                ...
                {guru.username && (
                  <span className="flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5" /> @{guru.username}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="pb-1">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border backdrop-blur-sm bg-white/20 text-white border-white/30">
                ...
                {guru.role?.charAt(0).toUpperCase() + guru.role?.slice(1) || 'Karyawan'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 px-6 pt-24 sm:pt-20 pb-6">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-white rounded-xl p-4 border border-blue-100">
            ...
            <p className="text-xl font-bold text-gray-800 truncate">{guru.username || '-'}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <InfoItem label="NUPTK" value={guru.nuptk || '-'} icon={<Award className="w-4 h-4 text-gray-400" />} />
            ...
            <InfoItem
              ...
              value={guru.jenis_karyawan || 'Guru'}
              icon={<User className="w-4 h-4 text-gray-400" />}
            />
            ...
            <InfoItem label="Role" value={guru.role || 'Karyawan'} icon={<GraduationCap className="w-4 h-4 text-gray-400" />} />
            <InfoItem label="Username" value={guru.username || '-'} icon={<User className="w-4 h-4 text-gray-400" />} />
        ...
        <div className="lg:col-span-2 card">
          ...
          {kelasWali.length === 0 ? (
            <div className="text-center py-8 text-gray-400">
              ...
              <p className="text-xs text-gray-300 mt-1">Tetapkan di menu Kelas</p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {kelasWali.map((k) => {
                const tingkatColor = k.tingkat === 'X' ? 'from-blue-500 to-blue-600' : k.tingkat === 'XI' ? 'from-emerald-500 to-emerald-600' : 'from-purple-500 to-purple-600'
                const tingkatBg = k.tingkat === 'X' ? 'bg-blue-50 border-blue-200' : k.tingkat === 'XI' ? 'bg-emerald-50 border-emerald-200' : 'bg-purple-50 border-purple-200'
                const tingkatIconColor = k.tingkat === 'X' ? 'text-blue-500' : k.tingkat === 'XI' ? 'text-emerald-500' : 'text-purple-500'
                return (
                  <div
                    key={k.id}
                    className={`card border ${tingkatBg} hover:shadow-md transition-all duration-200 relative overflow-hidden group`}
                  >
                    {/* Accent line */}
                    <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${tingkatColor}`} />

                    <div className="pt-2">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${tingkatColor} flex items-center justify-center shadow-sm shrink-0`}>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-gray-500 bg-white/80 px-2 py-0.5 rounded-full flex items-center gap-1">
                            <Users className="w-3 h-3" />
                            {k.jumlah_siswa ?? 0} siswa
                          </span>
                          <Link
                            to={`/kelas`}
                            className="text-xs text-annajah-600 hover:text-annajah-800 font-medium flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all"
                          >
                            Kelola
                          </Link>
                        </div>
