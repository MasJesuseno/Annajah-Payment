...
import { School, Volume2, VolumeX, ChevronLeft, ChevronRight, Play, Calendar, Sparkles, Maximize2, Minimize2, Monitor, MousePointer2 } from 'lucide-react'
...
export default function TvDisplay() {
  ...
  const userInteractedRef = useRef(false) // Track if user has clicked play for the first time
  const playerInitializedRef = useRef(false) // Track if player has been created
  ...
  // ─── Refs for latest values (anti stale closure in YouTube event handlers) ───
  const videosRef = useRef(videos)
  const currentVideoIndexRef = useRef(currentVideoIndex)
  const setCurrentVideoIndexRef = useRef(setCurrentVideoIndex)

  useEffect(() => { videosRef.current = videos }, [videos])
  useEffect(() => { currentVideoIndexRef.current = currentVideoIndex }, [currentVideoIndex])
  useEffect(() => { setCurrentVideoIndexRef.current = setCurrentVideoIndex }, [setCurrentVideoIndex])
  ...
  // Toggle fullscreen from button/F11
  ...
  useEffect(() => {
    const handleChange = () => {
      ...
      setIsFullscreen(fs)
      if (!fs && showClickOverlay) {
        // If user exits fullscreen via Esc before clicking overlay
        setShowClickOverlay(true)
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setShowFullscreenHint(false), 8000)
    return () => clearTimeout(timer)
  }, [showFullscreenHint])
  ...
  // ─── Buat & ganti video YouTube ───
  // Pertama kali: buat player baru.
  // Video berikutnya: panggil loadVideoById() pada player yang sama (tidak destroy/recreate).
  useEffect(() => {
    ...
    if (playerInitializedRef.current && playerRef.current) {
      // Player sudah ada → muat video baru (autoplay tetap jalan!)
    ...
    playerRef.current = new window.YT.Player(playerDiv, {
      ...
      playerVars: {
        autoplay: 1,
        mute: 1, // Always 1 for guaranteed autoplay
        controls: 0,
        showinfo: 0,
        rel: 0,
        modestbranding: 1,
      ...
      events: {
          ...
          if (event.data === 0) {
            ...
            if (curVideos.length === 1) {
              ...
              setTimeout(() => {
                ...
                setCurrentVideoIndexRef.current(nextIdx)
              }, 300)
            }
          }
          if (event.data === 1) {
        ...
        onError: () => {
          // Jika video error, lanjut ke video berikutnya
          const curVideos = videosRef.current
          const curIndex = currentVideoIndexRef.current
          if (curVideos?.length > 1) {
            setTimeout(() => {
  ...
  }, [currentVideoIndex, videos, startProgressTracker])
    ...
    >
      ...
      {showClickOverlay && (
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-white/90 mb-3 tracking-wide">Tampilan TV</h2>
          <p className="text-lg text-white/60 font-light mb-6">Klik di mana saja untuk layar penuh</p>
          <div className="flex items-center gap-3 text-white/30 text-sm">
            <kbd className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-xs font-mono text-annajah-300">F11</kbd>
            <span>atau</span>
            <kbd className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-xs font-mono text-annajah-300">Esc</kbd>
        ...
        {showFullscreenHint && !isFullscreen && (
          <div className="bg-black/60 backdrop-blur-xl border border-white/10 px-3 py-2 rounded-xl shadow-xl animate-fade-in flex items-center gap-2 pointer-events-none">
            ...
            <span className="text-xs text-white/70 whitespace-nowrap">
              Tekan <kbd className="px-1.5 py-0.5 bg-white/10 rounded text-annajah-200 text-[10px] font-mono">F11</kbd> layar penuh
            </span>
          </div>
        )}
        <button
      ...
      <style>{`
        ...
        /* Fullscreen styles */
        #tv-display-container:-webkit-full-screen {
          width: 100vw;
          height: 100vh;
        }
        #tv-display-container:fullscreen {
          width: 100vw;
          height: 100vh;
        }
        ...
        #tv-display-container:fullscreen:hover {
          cursor: default;
          animation: none;
        }
