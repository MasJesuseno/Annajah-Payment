...
const DB_CONFIG = {
  ...
  database: process.env.DB_NAME || 'dbannajah',
...
async function initDatabase() {
  ...
  try {
    ...
    // Add urutan column to tv_video if not exists (for ordering)
    try {
      await conn.execute(`ALTER TABLE tv_video ADD COLUMN urutan INT DEFAULT 0`);
    } catch (e) {}
