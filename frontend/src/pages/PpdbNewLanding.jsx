...
// ─── Main Component ───
export default function PpdbNewLanding() {
  ...
  useEffect(() => {
    Promise.allSettled([getLogoPublic(), getPengaturanPublic()]).then(([logoRes, sekolahRes]) => {
      ...
      if (sekolahRes.status === 'fulfilled' && sekolahRes.value.data) setSekolah(sekolahRes.value.data)
    })
  }, [])
  ...
  // Dynamic colors from pengaturan or defaults
        ...
        <div className="relative z-10 flex-1 flex items-center justify-center px-4">
          <div className="w-full max-w-4xl text-center">
            ...
            <p className="animate-blur-in anim-delay-4 text-base sm:text-lg text-[var(--ctp)] mb-10 max-w-xl mx-auto leading-relaxed">
              Sistem Penerimaan Peserta Didik Baru — Daftar secara online dengan verifikasi email, aman, dan mudah.
            </p>
          ...
          <AnimatedSection>
            <div className="max-w-md mx-auto">
              <TimelineStep
                ...
                icon={UserPlus}
                title="Daftar Online"
                desc="Klik tombol 'Daftar Sekarang' dan isi formulir pendaftaran dengan data diri yang lengkap dan benar termasuk email aktif."
              />
              <TimelineStep
                number={2}
                icon={Mail}
                title="Cek Email"
                desc="Setelah mendaftar, Anda akan menerima email berisi Nomor Pendaftaran dan Kode Rahasia 6 digit. Simpan baik-baik!"
              />
              <TimelineStep
                number={3}
                ...
                desc={(
                  <>
                    <span>Lakukan transfer biaya pendaftaran sebesar </span>
                    <strong className="text-[var(--cu)]">
                      Rp {(parseInt(sekolah?.biaya_pendaftaran || '350000') || 350000).toLocaleString('id-ID')}
                    </strong>
                    <span> ke rekening berikut:</span>
                    <div className="mt-2 p-3 bg-gray-50 rounded-xl border border-gray-200 space-y-1">
                      {(sekolah?.rekening_bank || sekolah?.rekening_nomor || sekolah?.rekening_atas_nama) ? (
                        ...
                        <p className="text-xs text-gray-400 italic">Informasi rekening akan diumumkan kemudian</p>
                      )}
                    </div>
                    <p className="text-xs text-gray-400 mt-1.5">Simpan bukti transfer untuk verifikasi.</p>
                    <p className="text-xs text-gray-500 mt-2">
                      Upload bukti transfer melalui edit data pendaftaran.{' '}
                      <Link to="/ppdbnew/edit" className="text-[var(--cu)] font-semibold hover:underline">
                        Klik Disini
                      </Link>
                    </p>
                  </>
              ...
              <TimelineStep
                ...
                desc="Pihak sekolah akan memverifikasi data pendaftaran Anda. Pantau status melalui menu 'Cek Hasil' menggunakan Nomor Pendaftaran dan Kode Rahasia."
              />
              <TimelineStep
                number={5}
                icon={CheckCircle}
                title="Pengumuman"
                desc="Hasil seleksi akan diumumkan melalui sistem. Jika ada kesalahan data, Anda bisa edit sendiri menggunakan Kode Rahasia."
                isLast
              />
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection delay={200}>
            <div className="mt-10">
              ...
              <div className="grid sm:grid-cols-4 gap-4">
                ...
                <Link
                  to="/ppdbnew/edit"
                  className="group bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl p-6 text-white text-center hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5"
                >
                  <BookOpen className="w-8 h-8 mx-auto mb-3 group-hover:scale-110 transition-transform duration-300" />
                  <h4 className="font-bold text-sm mb-1">Edit Data</h4>
                  <p className="text-xs text-white/70">Edit data atau upload ulang foto</p>
                </Link>
              </div>
            </div>
          </AnimatedSection>
          ...
          <div className="grid sm:grid-cols-3 gap-8 mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-xs">
                <li>
                  <Link to="/ppdbnew/daftar" className="hover:text-[var(--cu)] transition-colors">Pendaftaran Baru</Link>
                </li>
                <li>
                  <Link to="/ppdbnew/hasil" className="hover:text-[var(--cu)] transition-colors">Cek Hasil</Link>
                </li>
                <li>
                  <Link to="/ppdbnew/kartu" className="hover:text-[var(--cu)] transition-colors">Kartu Pendaftaran</Link>
                </li>
                <li>
                  <Link to="/ppdbnew/edit" className="hover:text-[var(--cu)] transition-colors">Edit Data</Link>
                </li>
              </ul>
            </div>
